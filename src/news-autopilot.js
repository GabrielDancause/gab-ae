...
function extractTags(title, description, paragraphs = []) {
  ...
  for (const match of matches) {
    ...
    // Skip very common capitalized words at start of sentences, months, days, etc.
    const stopWords = [
      'The', 'A', 'An', 'In', 'On', 'At', 'To', 'From', 'By', 'With', 'As', 'It', 'This', 'That', 'He', 'She', 'They', 'We', 'I', 'But', 'And', 'Or', 'If', 'When', 'What', 'Why', 'How',
      'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December',
      'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday',
      'According', 'However', 'Meanwhile', 'Earlier', 'Recently', 'Also', 'Still', 'Just', 'Here', 'There', 'Now', 'After', 'Before', 'During', 'Between', 'About', 'While', 'Since', 'Until'
    ];
    if (stopWords.includes(match)) continue;
...
function generateFaqs(title, category, sections) {
  ...
  // Extract a subject from the title (first capitalized word sequence after a potential start)
  const titleWords = title.split(' ').filter(w => /^[a-zA-Z0-9'-]+[.,:;]*$/.test(w));
  let subjectWords = [];
  for (let w of titleWords) {
    let cleanW = w.replace(/[.,:;]$/, '');
    if (/^[A-Z][a-z]+/.test(cleanW) && !['The', 'A', 'An', 'In', 'On', 'At', 'To', 'From', 'By', 'With', 'As', 'It', 'This', 'That', 'He', 'She', 'They', 'We', 'I', 'But', 'And', 'Or', 'If', 'When', 'What', 'Why', 'How', 'Is', 'Are', 'Was', 'Were', 'Be', 'Been', 'Being'].includes(cleanW)) {
      subjectWords.push(cleanW);
      if (subjectWords.length >= 3) break;
    } else if (subjectWords.length > 0) {
      break;
    }
  }
  const subject = subjectWords.length > 0 ? subjectWords.join(' ') : 'this development';

  // Helper to extract sentences from a specific section
  const getSentences = (sectionIndex) => {
    if (sectionIndex >= sections.length) return [];
    const text = sections[sectionIndex].paragraphs.join(' ');
  ...
  const s1Sentences = getSentences(0);
  const s2Sentences = getSentences(1);
  const s3Sentences = getSentences(2);

  // Q1
  if (s1Sentences.length >= 2) {
    faqs.push({
      q: `What happened with ${subject}?`,
      a: s1Sentences.slice(0, 2).join(' ')
    });
  }

  // Q2
  if (s2Sentences.length >= 2) {
    faqs.push({
      q: `Why does ${subject} matter?`,
      a: s2Sentences.slice(0, 2).join(' ')
    });
  }

  // Q3
  if (s3Sentences.length >= 2) {
    faqs.push({
      q: `What comes next for ${subject}?`,
      a: s3Sentences.slice(0, 2).join(' ')
    });
  }

  // Q4: Category specific (Optional)
  if (category === 'health' && s2Sentences.length >= 4) {
    faqs.push({
      q: `What are the health risks associated with ${subject}?`,
      a: s2Sentences.slice(2, 4).join(' ')
    });
  } else if (category === 'business' && s2Sentences.length >= 4) {
    faqs.push({
      q: `How will ${subject} affect the market?`,
      a: s2Sentences.slice(2, 4).join(' ')
    });
  }
...
function buildArticle(story, paragraphs) {
  ...
  while ((currentLength < 4000 || paragraphs.length < 6) && contextIndex < 10) {
